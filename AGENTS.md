...
| `src/acceptance.ts` | Goal/detected acceptance seed. Concrete-goal skip-discover. Finding class (`command_failed` / `command_refused` / `command_timeout` / `no_test` / `stale_workspace` / `host_crash`). Verify commands are allowlisted (named check ∪ `detectProjectCommands`). |
...
- Session id is `mvs_…` from structured fields (`exec.result.sessionId`, `cursor: sse1:session%3Amvs_…`). Assistant `YOUR SESSION ID:` does not bind. There is no leftover prose scrape.
- `plan` then `resume` continues the same run toward Accept. `plan` / `plan --run-id` still stop at PLAN_REVIEW. Verify timeout is `timedOut` / `command_timeout`, not generic exit 1. Lock steal requires a dead holder pid.
