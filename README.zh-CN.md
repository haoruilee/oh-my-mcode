...
| 你想要 | 跑这个 | 落地什么 |
| :--- | :--- | :--- |
| **CLI 进 PATH** | `git clone` → `npm install` → `npm link` | `oh-my-mcode` / `omm` |
| **健康检查** | `oh-my-mcode doctor` | 包装 + 宿主检查。不联网。 |
| **插件落盘** | `oh-my-mcode install` | 复制到 `~/.minimax/plugins/oh-my-mcode` |
...
| 🔌 | **MCP** | 无额外依赖的 stdio 工具：创建 / 查看 / 列出 run、status、verify、inspect。 |
| 👥 | **`team`** | TypeScript 扁平调度独立 builder。显式开启。默认仍是顺序 `max`。 |
| 🖥️ | **HUD** | `attach` / `status` 读同一份 `.minimax/runs/<id>/`。没有假装的 App 面板。 |
| 🩺 | **`doctor`** | 宿主 + 包装诚实检查。清单有、磁盘没有？那是错误。 |
...
[宿主现状](docs/host-reality.zh-CN.md) · [架构](docs/architecture.zh-CN.md) · [路线图](docs/roadmap.md)
