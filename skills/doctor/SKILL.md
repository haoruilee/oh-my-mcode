...
1. Run the package checker (this is the real test):
   `oh-my-mcode doctor --package-only` or `node <plugin-root>/scripts/doctor.mjs`
...
3. Check the drop-in path `~/.minimax/plugins/oh-my-mcode` (and mention `~/.mavis` is a symlink to `~/.minimax` on current hosts). If the folder is missing, point at `scripts/install.sh` or `scripts/install.ps1`.
